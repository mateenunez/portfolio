...
const TickerTrack: React.FC<TickerTrackProps> = ({
  children,
  direction = "toRight",
  speed = 30,
}) => {
...
export function Ticker() {
  ...
  const [tickerImages, setTickerImages] = useState<{
    [key: string]: TickerImage[];
  }>({});
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        ...
        const processedImages = Object.entries(data).reduce(
          (acc, [row, images]) => ({
            ...acc,
            [row]: images.map((item) => ({
